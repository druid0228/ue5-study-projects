	...
	if (HasHit)
	{
		...
		if (HitActor->ActorHasTag("CollectableItem"))
		{
			...
			if (CollectableItem)
			{
				...
				//UE_LOG(LogTemp, Display, TEXT("collectable item name : %s"),*CollectableItem->ItemName);
			}
			else
			{
		...
		else if (HitActor->ActorHasTag("Lock"))
		{
			...
			if (LockActor)
			{
				//UE_LOG(LogTemp, Display, TEXT("lock ket item name : %s"), *lock->KeyItemName);
				...
				else
				{
					...
					// Add the KeyItemName into ItemList;
					// Deactivate the lock
		...
		{
			UE_LOG(LogTemp, Display, TEXT("HitActor has not interact tag"));
		}
